...
func TestPageSourceTaskCanUseScheduleWithoutCodeArtifact(t *testing.T) {
	app := newTestControlApp(t)
	token := loginAndGetToken(t, app)

	taskRec := doJSONRequest(t, app, http.MethodPost, "/api/v1/tasks", token, map[string]any{
		"name":       "example page source",
		"type":       "page",
	...
	if taskRec.Code != http.StatusOK {
		t.Fatalf("create page task status=%d body=%s", taskRec.Code, taskRec.Body.String())
	}
	var task common.Task
	if err := json.Unmarshal(taskRec.Body.Bytes(), &task); err != nil {
		t.Fatalf("decode task: %v", err)
	...
	if steps[0].Type != "page" || steps[0].SourceURL != task.SourceURL {
		t.Fatalf("source schedule did not preserve page task: %#v", steps[0])
	}
}
