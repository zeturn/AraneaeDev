...
const tabsList = computed(() => {
	...
	const tabs = [
		{name: t('程序任务'), url: `/aprons/workplaces/${id}/tasks`},
		{name: t('创建任务'), url: `/aprons/workplaces/${id}/tasks/create`},
	];
	if (route.params.taskId) {
		tabs.push({name: t('任务设置'), url: `/aprons/workplaces/${id}/tasks/${route.params.taskId}/settings`});
		tabs.push({name: t('运行记录'), url: `/aprons/workplaces/${id}/tasks/${route.params.taskId}/runs`});
	}
	return tabs;
});
</script>
...
<template>
	<section class="w-full min-w-0 overflow-x-hidden px-4">
		...
		<p class="text-gray-500 text-sm py-2">
			{{ $t('任务是一次性的实时自动化操作，如果需要定时或链式操作，请使用计划。') }}
		</p>
