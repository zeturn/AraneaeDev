...
<template>
	<Node>
		<div class="container">
			<h1 class="text-3xl font-semibold mb-6 text-gray-500">{{ $t('节点设置') }}</h1>
			<!-- 节点设置（更新/删除） -->
			<div v-if="form" class="max-w-4xl p-6 bg-white">
				...
				<form class="grid grid-cols-1 gap-6" @submit.prevent="updateNode">
					...
					<div>
						<label class="block mb-2 text-gray-700 text-sm font-medium">{{ $t('IP 地址') }}</label>
					...
					<div>
						<label class="block mb-2 text-gray-700 text-sm font-medium">{{ $t('端口') }}</label>
					...
					<div>
						...
						<input v-model="form.rpc_url"
						       ...
						       type="text"
						       :placeholder="$t('请输入 RPC URL')"
						/>
					</div>
					<div>
						<label class="block mb-2 text-gray-700 text-sm font-medium">{{ $t('Celery 队列') }}</label>
						<input v-model="form.celery_queue"
						       class="field-input"
						       type="text"
						       :placeholder="$t('请输入 Celery 队列')"
						/>
					</div>
					<div class="flex items-center">
						<CheckboxSquareField id="enabled" v-model="form.is_enabled">{{ $t('启用') }}</CheckboxSquareField>
					</div>
